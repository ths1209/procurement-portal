          ...
          <div key={obj.id} className="rounded-2xl overflow-hidden"
            ...
            {isExpanded && (
              <div style={{ borderTop: '1px solid var(--border)' }}>
                {obj.krs.length === 0 && (
                  <div className="px-5 py-8 text-center text-sm border-b"
                    style={{ color: 'var(--muted)', borderColor: 'var(--border)' }}>暂无 KR</div>
                )}
                {obj.krs.map((kr, ki) => (
                ...
                {/* 每个O末尾的其他工作行 */}
                <KRSection
                  kr={{ id: otherKey(obj.id), desc: '其他工作' }}
                  ki={obj.krs.length}
                  typeColor={OTHER_CLR}
                  filteredPeriods={filteredPeriods} allReports={allReports}
                  historyEntries={krHistories[otherKey(obj.id)]}
                  historyExpanded={expandedHist.has(otherKey(obj.id))}
                  historyLoading={loadingHist === otherKey(obj.id)}
                  onToggleHistory={() => toggleKRHistory(otherKey(obj.id))}
                  onCellClick={(periodId, group, periodLabel) =>
...
function ReportPanel({ annualOkr, quarterlyOkr, periods, allReports, selectedPeriod, onPeriodChange, myGroup, profile, onSaved }) {
  ...
  async function handleSave(newSubmitStatus) {
    ...
    try {
      const otherFakes = [
        ...annualOkr.objectives.map(o => ({ kr: { id: otherKey(o.id), desc: '其他工作' } })),
        ...quarterlyOkr.objectives.map(o => ({ kr: { id: otherKey(o.id), desc: '其他工作' } })),
      ]
      const changes = buildChangeDiff(krData, draft, [...allKRs, ...otherFakes])
      const payload = {
        ...draft,
        _meta: {
      ...
      {!selectedPeriod ? (
        ...
        <div className="space-y-3">
          {allKRs.map(({ typeLabel, oi, obj, kr }, idx) => {
            ...
            const isLastKROfObj = idx === allKRs.length - 1 || allKRs[idx + 1].obj.id !== obj.id
            const d    = draft[kr.id] || {}
            const prev = prevReport[kr.id] || {}
            const tc   = typeLabel === '年度' ? ACCENT : Q_ACCENT
            const ok   = otherKey(obj.id)
            return [
              <div key={kr.id}>
                {showHead && (
                ...
                <div key={`other-${obj.id}`}>
                  ...
                  <div className="card p-4 space-y-3"
                    ...
                    <div className="flex items-center gap-3 ml-7">
                      ...
                      <div className="flex gap-2">
                        {['notstart', 'progress', 'done'].map(s => {
                          const cfg = STATUS_CFG[s]; const active = (draft[ok] || {}).status === s
                          return (
                            <button key={s} onClick={() => update(ok, 'status', s)}
                              className="press px-3 py-1.5 rounded-xl text-[11px] font-bold transition-all"
                              style={active
                                ? { background: cfg.solid, color: cfg.text, boxShadow: `0 2px 8px ${cfg.solid}55` }
                                : { background: 'var(--surface)', color: 'var(--text)', border: '1px solid var(--border)', opacity: 0.6 }}>
                    ...
                    <div className="ml-7">
                      <textarea value={(draft[ok] || {}).content || ''} onChange={e => update(ok, 'content', e.target.value)}
                        className="field text-[13px]" rows={3} style={{ resize: 'vertical' }}
                        placeholder={`描述 ${myGroup} 本O其他工作…`} />
                    </div>
                    <div className="ml-7">
                      <div className="flex items-center gap-2 mb-1.5">
                        <span className="text-[11px] font-semibold" style={{ color: 'var(--text)', opacity: 0.65 }}>支撑附件</span>
                        {!isSubmitted && (
                          <>
                            <button onClick={() => fileInputRefs.current[ok]?.click()} disabled={!!uploadingKr}
                              className="press flex items-center gap-0.5 px-2 py-0.5 rounded-lg text-[10px] font-semibold disabled:opacity-40"
                              style={{ background: 'rgba(100,116,139,0.07)', color: OTHER_CLR, border: '1px solid rgba(100,116,139,0.2)' }}>
                              {uploadingKr === ok ? <Loader2 className="w-2.5 h-2.5 animate-spin" /> : <Plus className="w-2.5 h-2.5" />}上传
                            </button>
                            <input ref={el => { fileInputRefs.current[ok] = el }} type="file" multiple hidden
                              onChange={e => { handleKRFileUpload(ok, Array.from(e.target.files)); e.target.value = '' }} />
                          </>
                        )}
                      </div>
                      {(krAtts[ok] || []).length > 0 && (
                        <div className="space-y-1">
                          {(krAtts[ok] || []).map(att => (
                            <div key={att.token} className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[11px]"
                              ...
                              <FileText className="w-3 h-3 shrink-0" style={{ color: 'var(--muted)' }} />
                              <span className="flex-1 truncate" style={{ color: 'var(--text)' }}>{att.displayName}</span>
                              <span className="font-mono shrink-0 text-[10px]" style={{ color: 'var(--muted)' }}>{fmtFileSize(att.size)}</span>
                              {att.presignedUrl && (
                                <a href={att.presignedUrl} target="_blank" rel="noreferrer"
                                  className="press px-1.5 py-0.5 rounded text-[10px]"
                              ...
                              {!isSubmitted && (
                                <button onClick={() => handleKRFileDelete(att.token)} disabled={deletingToken === att.token}
                                  className="press px-1.5 py-0.5 rounded text-[10px] disabled:opacity-40"
                                  style={{ background: 'rgba(244,63,94,0.07)', color: '#E11D48' }}>
                                  {deletingToken === att.token ? '…' : '删除'}
                                </button>
